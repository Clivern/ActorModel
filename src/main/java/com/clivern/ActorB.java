...
/** ActorB Class */
public class ActorB extends AbstractBehavior<ActorB.Payload> {

    private Injector injector;

    private final ActorRef<ChildActorD.Payload> childActorD;
    ...
    public static Behavior<Payload> create(Injector injector) {
        return Behaviors.setup(context -> new ActorB(context, injector));
    }

    private ActorB(ActorContext<Payload> context, Injector injector) {
        super(context);
        this.injector = injector;

        //#create-sub-actors
        childActorD = context.spawn(ChildActorD.create(injector), "childActorD");
        //#create-sub-actors
    }
    ...
    private Behavior<Payload> onMessage(Payload payload) {
        ...
        if (!payload.message.equals("From ChildActorD to ActorB")) {
            childActorD.tell(new ChildActorD.Payload("From ActorB to ChildActorD"));
        }
